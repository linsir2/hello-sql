...
- 存储（B 的实现入口）：`storage/__init__.py` 暴露 `class Storage`；方法清单见 [docs/contract-v1.md](docs/contract-v1.md) 第 3 节
...
from storage import Storage         # B：表级 CRUD，Storage("data") 实例化
from runner import Runner           # C：执行与 REPL

# main.py 装配（唯一允许同时接触三家的地方）
storage = Storage("data")
runner = Runner(storage=storage, parse=parse)
```

运行期由 runner 内部调用 storage 的 describe / scan / insert /
update_row / delete_row。compiler、storage、runner 之间互不 import，
只允许 import contracts 里的共享数据格式。
